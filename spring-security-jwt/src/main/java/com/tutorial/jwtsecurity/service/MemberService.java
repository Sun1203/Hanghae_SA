...
public class MemberService {
    ...
    public MemberResponseDto signup(MemberRequestDto memberRequestDto) {
        ...
        // 아이디, 비밀번호 형식확안
        String idReq = "^(?=.*[A-Za-z])(?=.*\\d)[A-Za-z\\d]{4,12}$";
        String pwReq = "^(?=.*[A-Za-z])(?=.*\\d)[A-Za-z\\d]{4,32}$";

        // 아이디 형식
        if (!Pattern.matches(idReq, memberRequestDto.getNickname())) {
            throw new IllegalArgumentException("아이디 형식이 틀립니다");
        }

        // 비밀번호 형식
        if (!Pattern.matches(pwReq, memberRequestDto.getPassword())) {
            throw new IllegalArgumentException("비밀번호 형식이 틀립니다");
        }

        // 비밀번호와 비밀번호확인이 두개가 같은지 확인
